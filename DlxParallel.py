...
def solve_sudoku(size, grid):
    ...
    R, C = size
    N = R * C
    #Creates the set of all constraints for sudoku
    X = ([("rc", rc) for rc in product(range(N), range(N))] +
         [("rn", rn) for rn in product(range(N), range(1, N + 1))] +
         [("cn", cn) for cn in product(range(N), range(1, N + 1))] +
         [("bn", bn) for bn in product(range(N), range(1, N + 1))])
    Y = dict()
    #Creates the set of all possiblities for sudoku
    for r, c, n in product(range(N), range(N), range(1, N + 1)):
        ...
        Y[(r, c, n)] = [
            ...
            ("cn", (c, n)),
            ("bn", (b, n))]
    #Models the problem as an exact cover where X is a dict with:
    #key=constraint and value=all possibilities that satisfy that constraint
    X, Y = exact_cover(X, Y)

    #Selects the possibilities that must be used. ie. the spaces that are filled in
...
def select(X, Y, r):
    with concurrent.futures.ThreadPoolExecutor(max_workers=1000000) as executor:
        cols = executor.map(select_parallel,repeat(X),repeat(Y), Y[r])
    return functools.reduce(list.__add__, cols)


def select_parallel(X,Y,j):
    cols = []
    for i in X[j]:
        for k in Y[i]:
            if k != j:
